...
module tb_Trafficlights_FSM();
    ...
    initial begin
        ...
        clk = 0;
        reset = 1;
        TAORB = 1; // Start with traffic on Street A [cite: 21]
        ...
        #20;
        TAORB = 0; // No more traffic at A, transition to S1 [cite: 22]
        
        // Stay in S1 (Yellow) for 5 clock cycles 
        repeat (6) @(posedge clk); 

        // --- Test Scenario 2: Transition from S2 to S3 ---
        TAORB = 1; // Traffic returns to A, transition to S3 [cite: 26]
        
        // Stay in S3 (Yellow) for 5 clock cycles [cite: 29]
        repeat (6) @(posedge clk);
